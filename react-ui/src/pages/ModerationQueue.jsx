...
export default function ModerationQueue() {
  ...
  return <>
    ...
    {!loading && !queueError && <div className="moderation-list">{submissions.map((item) => <article className="moderation-card" key={item.id}>
      ...
      <h2>{item.proposal_kind === "enrichment" ? "collection enrichment" : `${item.record_type} ${item.submission_type}`}</h2><p>{item.explanation}</p>
      ...
      {item.status === "submitted" && (item.submitter_clerk_user_id === appUser?.clerk_user_id ? <StatusMessage>You cannot review your own submission.</StatusMessage> : <><div className="actions">{(item.proposal_kind !== "enrichment" || appUser?.role === "admin") && <button type="button" onClick={() => setPendingAction({ id: item.id, action: "approve" })}>Approve</button>}<button className="secondary" type="button" onClick={() => setPendingAction({ id: item.id, action: "request_changes" })}>Request changes</button><button className="danger" type="button" onClick={() => setPendingAction({ id: item.id, action: "reject" })}>Reject</button></div>{item.proposal_kind === "enrichment" && appUser?.role !== "admin" && <StatusMessage>Administrator approval is required for canonical promotion.</StatusMessage>}</>)}
