...
def upgrade():
    ...
    if not sa.inspect(connection).has_table("archive_state"):
        op.create_table(
            "archive_state",
            ...
            sa.CheckConstraint(
                "version > 0", name="ck_archive_state_version_positive"
            ),
            mysql_engine="InnoDB",
    ...
    existing_row = connection.execute(
        sa.text("SELECT version FROM archive_state WHERE id = 1")
    ).first()
